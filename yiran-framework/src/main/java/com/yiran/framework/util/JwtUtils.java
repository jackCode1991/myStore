...
public class JwtUtils {
    ...
    public static String createToken(MiniProgrameUser user) throws Exception {
        try {
        	String obj=JSONObject.toJSONString(user);
            Algorithm algorithm = Algorithm.HMAC256(SECRET);
            JWTCreator.Builder builder = JWT.create()
                    .withIssuer(ISSUER)
                    .withExpiresAt(DateUtils.addSeconds(new Date(), Global.getJwtOutTime())).withSubject(obj);
            return builder.sign(algorithm);
        } catch (IllegalArgumentException | UnsupportedEncodingException e) {
            logger.error("生成token失败",e);
            throw new Exception("生成token失败");
    ...
    public static String createWeiXinToken(WeixinUser user) throws Exception {
        try {
        	String obj=JSONObject.toJSONString(user);
            Algorithm algorithm = Algorithm.HMAC256(SECRET);
            JWTCreator.Builder builder = JWT.create()
