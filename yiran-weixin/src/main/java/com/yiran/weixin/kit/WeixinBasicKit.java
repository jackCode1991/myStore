...
public class WeixinBasicKit {
	...
	public static void setWeixinContext() {
		...
		//小程序
		String mpAppId = weixinSettingMapper.getValueByKey(WeixinSetting.KEY_WEIXIN_SMALL_PROGRAM_APPID).getWeixinValue();
		logger.info("微信小程序appId:"+mpAppId);
		WeixinContext.getInstance().setMiniAppId(mpAppId);
		String mpSecret = weixinSettingMapper.getValueByKey(WeixinSetting.KEY_WEIXIN_SMALL_PROGRAM_APPSECRET).getWeixinValue();
		logger.info("微信小程序appSecurt:"+mpAppId);
		WeixinContext.getInstance().setMiniAppSecurt(mpSecret);
	}
